...
import { randomizeBusinessDayRecord, randomizeBusinessDayRecordLegacy } from '../../domain/businessDay/randomizeBusinessDay.js';
...
export async function loadD2BusinessDayDefinition({
  ...
  seed = null,
  randomizationVersion = 2,
} = {}) {
  try {
    const response = await fetchImpl(url);
    if (!response.ok) return { ok: false, error: { code: 'D2_DEFINITION_HTTP', message: `D2 정의 HTTP ${response.status}` } };
    const record = await response.json();
    const randomize = randomizationVersion === 1 ? randomizeBusinessDayRecordLegacy : randomizeBusinessDayRecord;
    const rolled = seed === null ? record : randomize(record, { seed });
