...
도구는 **게임이 실제 로드하는 `content/` 데이터를 직접 편집**합니다. 도구에서 바꾼 값이 곧 게임 값입니다.

- 조리 구간·손님 유형별 인내심·간격·팁·판매가 등을 슬라이더로 조절 (값은 `content/`, 상·하한·단위는 JSON Schema에서)
- **시뮬레이션 실행** → `src/state/businessDay.js`의 정식 영업일 루프로 한 영업일을 돌려 총수익·서빙·이탈·평균 대기·품질 분포 표시
- 실행 전 `src/content/rules.js`의 런타임 검증(게임과 동일)을 거침
- 직전 실행 대비 변화(초록/빨강 화살표)로 수치 영향 비교
- **변경된 content 내보내기** → 내려받은 파일로 `content/`를 교체하면 게임에 반영

시뮬레이터는 게임과 **같은 루프 로직**(`businessDay`)을 재사용해 "도구에선 되는데 게임에선 다른"
괴리가 없습니다. 값은 `candidate`이며 `approved`만 게임 기본값으로 로드됩니다.

**한계**: 저장이 파일 다운로드 후 수동 교체 방식입니다(`SYS-005` 저장 방식 확정 후 개선). 도구는
D1 영업일만 다룹니다.
