...
describe('D5 전체 영업 종단', () => {
  it('9개 주문을 제공·정리·정산하고 D5 완료를 한 번만 저장한다', async () => {
    ...
    definition.waves.forEach((wave, waveIndex) => {
      ...
      for (const customer of wave.customers) {
        expect(dispatch(port, `d5:accept:${customer.order.id}`, D1_UI_INTENT.ACCEPT_ORDER, { orderId: customer.order.id }).ok).toBe(true);
        customer.order.lines.forEach((line, lineIndex) => {
          for (let itemIndex = 0; itemIndex < line.quantity; itemIndex += 1) {
            expect(dispatch(port, `d5:serve:${customer.order.id}:${lineIndex}:${itemIndex}`, D1_UI_INTENT.SERVE_ITEM, {
      ...
      cleanupAll(port, `d5:cleanup:${waveIndex}`);
    });

    advanceTo(port, definition.sessionTargetMs);
    cleanupAll(port, 'd5:cleanup:final');
    expect(dispatch(port, 'd5:charcoal', D1_UI_INTENT.LOWER_CHARCOAL).ok).toBe(true);
    D1_SETTLEMENT_STEPS.forEach((_, index) => {
      expect(dispatch(port, `d5:settlement:${index}`, D1_UI_INTENT.REVEAL_SETTLEMENT_STEP).ok).toBe(true);
    });
    ...
    expect(completed.settlement).toMatchObject({
      customers: { visited: 9, lost: 0, cleanedSeats: 9 },
      orders: { accepted: 9, completed: 9, abandoned: 0 },
      quality: { Perfect: 17, Good: 0, OK: 0, Fail: 0 },
